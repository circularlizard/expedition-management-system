...
## 6. Signup Test Data Seeder

To allow scale and performance testing of the Signups grid, we introduce a WP-CLI seeding script:

*   **Location**: `bin/seed-signups.php`
*   **Execution Command**:
    ```bash
    wp eval-file wp-content/plugins/ems-plugin/bin/seed-signups.php -- [count]
    ```
*   **Arguments**:
    *   `count` (int, optional): Number of signup records to generate (defaults to `50`).
*   **Behavior & Rules**:
    1.  *Cleans Data*: Deletes any existing mock signup rows generated by previous seeder runs (safely isolated by checking specific mock `form_submission_id` ranges) to remain idempotent.
    2.  *Parent User Accounts*: Ensures mock parent user accounts (`wp_users` with the role `ems_parent`) exist to act as signers.
    3.  *Varied Linkage Buckets*:
        *   **Directly Linked**: Signups populated with a valid `scout_id` corresponding to an explorer in `ems_osm_explorers`.
        *   **Fuzzy Matchable**: Signups with `scout_id = null`, but with email or first/last names matching an existing synced explorer record (tests the "Proposed Link" dashboard UI).
        *   **New Recruits**: Signups with no matching explorer record (unlinked state).
    4.  *State Distribution*:
        *   *Levels*: 50% Bronze, 35% Silver, 15% Gold.
        *   *Payments*: 70% `'paid'`, 25% `'pending'`, 5% `'failed'`.
        *   *Signup Status*: 60% `'pending'` (active), 30% `'processed'` (completed), 10% `'archived'`.
    5.  *Audit Simulation*: For signups seeded with status `'processed'`, populate `processed_by` with the administrator user ID and `processed_at` with a mock timestamp within the last 30 days.
    6.  *Preferences Payloads*: Generates realistic, randomized JSON strings stored in `expedition_preferences`:
        ```json
        {
          "preferred_dates": ["2026-06-13", "2026-06-20"],
          "teammate_preferences": ["Alice Smith", "Bob Jones"],
