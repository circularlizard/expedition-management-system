...
class TutorLMS_ClientTest extends EMSTestCase {
    ...
    public function test_get_enrollment_matrix_completes_lesson_only_course_without_topic(): void {
        ...
        $wpdb->shouldReceive( 'get_results' )->andReturnValues([
            [ (object) [ 'post_author' => 7, 'post_parent' => 42 ] ], // Enrollments
            [], // Explicit
            [ (object) [ 'content_id' => 101, 'content_type' => 'tutor_lesson', 'course_id' => 42 ] ], // Content
            [ (object) [ 'user_id' => 7, 'meta_key' => '_tutor_completed_lesson_id_101' ] ], // Step 4
            [], // Step 4b
        ]);
    ...
    public function test_get_enrollment_matrix_completes_assignment_only_course_when_submitted(): void {
        ...
        $wpdb->shouldReceive( 'get_results' )->andReturnValues([
            [ (object) [ 'post_author' => 7, 'post_parent' => 42 ] ], // Enrollments
            [], // Explicit
            [ (object) [ 'content_id' => 200, 'content_type' => 'tutor_assignments', 'course_id' => 42 ] ], // Content
            [ (object) [ 'user_id' => 7, 'meta_key' => '_tutor_completed_lesson_id_200' ] ], // Step 4
            [], // Step 4b
            [], // Step 5.5 (posts)
            [], // Step 5.5.5 (comments)
        ]);
    ...
    public function test_get_enrollment_matrix_in_progress_when_assignment_not_submitted(): void {
        ...
        $wpdb->shouldReceive( 'get_var' )->andReturn( null );
        $wpdb->shouldReceive( 'get_results' )->andReturn( [] ); // Diagnostics
    ...
    public function test_get_enrollment_matrix_completes_lesson_and_assignment_course_when_both_done(): void {
        ...
        $wpdb->shouldReceive( 'get_results' )->andReturnValues([
            ...
            [ (object) [ 'user_id' => 7, 'meta_key' => '_tutor_completed_lesson_id_101' ] ], // Step 4
            [], // Step 4b
            [ (object) [ 'post_author' => 7, 'post_parent' => 200 ] ], // Step 5.5
            [], // Step 5.5.5
        ]);
        $wpdb->shouldReceive( 'get_var' )->andReturn( null );
    ...
    public function test_get_enrollment_matrix_completes_lesson_course_via_reading_info_meta(): void {
        ...
        $wpdb->shouldReceive( 'get_results' )->andReturnValues([
            ...
            [ (object) [ 'content_id' => 101, 'content_type' => 'tutor_lesson', 'course_id' => 42 ] ], // Content
            [], // Step 4
            [ (object) [ 'user_id' => 7, 'meta_key' => '_tutor_reading_info_42', 'meta_value' => serialize( [ 101 => true ] ) ] ], // Step 4b
        ]);
    ...
    public function test_get_enrollment_matrix_uses_cb_content_usage_table(): void {
        ...
        $wpdb = $this->setup_wpdb_mock();
        $wpdb->shouldReceive( 'get_results' )->andReturnValues([
            [ (object) [ 'post_author' => 7, 'post_parent' => 42 ] ], // Step 1
            [], // Step 2
            [ (object) [ 'content_id' => 300, 'content_type' => 'tutor_assignments', 'course_id' => 42 ] ], // Step 3
            [], // Step 4
            [], // Step 4b
            [], // Step 5.5
            [], // Step 5.5.5
            [ (object) [ 'user_id' => 7, 'content_id' => 300 ] ], // usage table results
        ]);
        $wpdb->shouldReceive( 'get_var' )->andReturn( 'wp_tutor_cb_content_usage' );
    ...
    public function test_get_enrollment_matrix_completes_assignment_via_comments(): void {
        ...
        $wpdb->shouldReceive( 'get_results' )->andReturnValues([
            ...
            [], // Step 4b
            [], // Step 5.5
            [ (object) [ 'user_id' => 7, 'comment_post_ID' => 300 ] ], // Step 5.5.5
        ]);
        $wpdb->shouldReceive( 'get_var' )->andReturn( null );
    ...
    private function setup_wpdb_mock() {
