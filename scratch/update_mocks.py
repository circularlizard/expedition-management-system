import openpyxl
import json
from datetime import datetime

# Load workbook
wb = openpyxl.load_workbook('docs/Members - 2026-07-23.xlsx')
sheet = wb.active

# Parse rows
members_items = []
member_details = {}
scout_id_counter = 3417257
section_id = 99001

patrol_map = {
    'S3': '99201',
    'S4': '99202',
    'S5': '99203',
    'S6': '99204',
    'Post S6': '99205'
}

ref_date = datetime(2026, 6, 13)

for r in range(3, sheet.max_row + 1):
    fname = sheet.cell(row=r, column=1).value
    lname = sheet.cell(row=r, column=2).value
    dob_val = sheet.cell(row=r, column=3).value
    patrol_val = sheet.cell(row=r, column=4).value
    
    # Column indices:
    # Primary Contact 1 Email 1 is 1-based column 16 (index 15)
    # Member Email 1 is 1-based column 63 (index 62)
    parent_email_val = sheet.cell(row=r, column=16).value
    email_val = sheet.cell(row=r, column=63).value
    
    if not fname or not lname:
        continue
        
    scout_id = str(scout_id_counter)
    scout_id_counter += 1
    
    if isinstance(dob_val, datetime):
        dob_str = dob_val.strftime('%Y-%m-%d')
        years = ref_date.year - dob_val.year
        months = ref_date.month - dob_val.month
        if months < 0:
            years -= 1
            months += 12
        age_str = f"{years} / {months}"
    else:
        dob_str = "2008-01-01"
        age_str = "18 / 5"
        
    patrol_name = patrol_val if patrol_val else 'S3'
    patrol_id = patrol_map.get(patrol_name, '99201')
    
    members_items.append({
        "scoutid": scout_id,
        "firstname": fname,
        "lastname": lname,
        "patrol": patrol_name,
        "patrolid": patrol_id,
        "sectionid": section_id,
        "dob": dob_str,
        "age": age_str,
        "_filterString": f"{fname.lower()} {lname.lower()}"
    })
    
    member_details[scout_id] = {
        "email": email_val if email_val else f"scout.{scout_id}@example-ems.test",
        "parent_email": parent_email_val if parent_email_val else f"parent.{scout_id}@example-ems.test"
    }

# 1. Write osm-list-of-members.json
list_payload = {
    "identifier": "scoutid",
    "photos": True,
    "items": members_items
}
with open('tests/mocks/osm-list-of-members.json', 'w') as f:
    json.dump(list_payload, f, indent=4)

# 2. Write osm-member-detail.json
with open('tests/mocks/osm-member-detail.json', 'w') as f:
    json.dump(member_details, f, indent=4)

# 3. Write osm-patrols.json
patrols_payload = {
    "patrols": [
        {
            "patrolid": "-2",
            "sectionid": str(section_id),
            "name": "Leaders",
            "active": "1",
            "points": "0",
            "census_costs": False
        },
        {
            "patrolid": "99201",
            "sectionid": str(section_id),
            "name": "S3",
            "active": "1",
            "points": "0",
            "census_costs": False
        },
        {
            "patrolid": "99202",
            "sectionid": str(section_id),
            "name": "S4",
            "active": "1",
            "points": "0",
            "census_costs": False
        },
        {
            "patrolid": "99203",
            "sectionid": str(section_id),
            "name": "S5",
            "active": "1",
            "points": "0",
            "census_costs": False
        },
        {
            "patrolid": "99204",
            "sectionid": str(section_id),
            "name": "S6",
            "active": "1",
            "points": "0",
            "census_costs": False
        },
        {
            "patrolid": "99205",
            "sectionid": str(section_id),
            "name": "Post S6",
            "active": "1",
            "points": "0",
            "census_costs": False
        }
    ]
}
with open('tests/mocks/osm-patrols.json', 'w') as f:
    json.dump(patrols_payload, f, indent=4)

print(f"Successfully generated mock files with {len(members_items)} members.")
