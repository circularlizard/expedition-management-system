	...
	public function render(): void {
		...
		foreach ( $students as $student ) {
			...
			foreach ( $courses as $course ) {
				...
				$sort_val = match ( $status ) {
					'complete' => 2, 'in_progress' => 1, default => 0 };
                // phpcs:ignore WordPress.Security.EscapeOutput
		...
		if ( $diag ) {
			...
			printf(
				...
				esc_html( implode( ', ', $diag['assignment_done_ids'] ?? array() ) ?: '(none)' )
			);
			...
			if ( ! empty( $diag['cb_sample'] ) ) {
				...
				foreach ( $diag['cb_sample'] as $row ) {
					...
					foreach ( (array) $row as $val ) {
						printf( '<td><code>%s</code></td>', esc_html( substr( (string) $val, 0, 60 ) ) ); }
					echo '</tr>';
				}
				echo '</tbody></table>';
			} else {
			...
			if ( ! empty( $diag['cb_assign_rows'] ) ) {
				...
				foreach ( $diag['cb_assign_rows'] as $row ) {
					echo '<tr>';
					foreach ( (array) $row as $val ) {
						printf( '<td><code>%s</code></td>', esc_html( (string) $val ) ); }
					echo '</tr>';
				}
				echo '</tbody></table>';
			} else {
