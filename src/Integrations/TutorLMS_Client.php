...
class TutorLMS_Client {
	...
	public function get_enrollment_matrix( array $user_ids, array $course_ids ): array {
        ...
        // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
		$enrolled_rows = $wpdb->get_results(
			$wpdb->prepare(
        ...
        // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
		$explicit_rows = $wpdb->get_results(
			$wpdb->prepare(
		...
		$content_rows = $wpdb->get_results(
			$wpdb->prepare(
             ...
             AND   c.post_type   IN ('tutor_lesson', 'lesson', 'tutor_quiz', 'tutor_assignments', 'tutor_assignment')
             AND   t.post_parent IN ({$c_ph})
             UNION
             SELECT c.ID as content_id, c.post_type as content_type, c.post_parent as course_id
             FROM {$wpdb->posts} c
             WHERE c.post_type   IN ('tutor_lesson', 'lesson', 'tutor_quiz', 'tutor_assignments', 'tutor_assignment')
             AND   c.post_status IN ('publish', 'private')
		...
		$lesson_done     = array();
		$assignment_done = array();
		$all_lesson_set  = ! empty( $all_lesson_ids ) ? array_flip( $all_lesson_ids ) : array();
		$all_assign_set  = ! empty( $all_assignment_ids ) ? array_flip( $all_assignment_ids ) : array();
		$all_quiz_set    = ! empty( $all_quiz_ids ) ? array_flip( $all_quiz_ids ) : array();

		if ( ! empty( $all_lesson_ids ) || ! empty( $all_assignment_ids ) || ! empty( $all_quiz_ids ) ) {
            // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
			$lesson_meta_rows = $wpdb->get_results(
				$wpdb->prepare(
                 ...
                 FROM {$wpdb->usermeta}
                 WHERE user_id IN ({$u_ph})
                 AND   meta_key LIKE '_tutor_completed_%%'",
					...$user_ids
				)
			);
			foreach ( $lesson_meta_rows as $row ) {
				...
				if ( ! $lid ) {
					$lid = (int) str_replace( '_tutor_completed_assignment_id_', '', $row->meta_key );
				}
				if ( ! $lid ) {
					$lid = (int) str_replace( '_tutor_completed_quiz_id_', '', $row->meta_key );
				}
				if ( ! $lid ) {
					continue;
				}
				...
				} elseif ( isset( $all_assign_set[ $lid ] ) ) {
					$assignment_done[ (int) $row->user_id ][ $lid ] = true;
				} elseif ( isset( $all_quiz_set[ $lid ] ) ) {
					$quiz_done[ (int) $row->user_id ][ $lid ] = true;
				}
			}
		}
		...
		if ( ! empty( $course_ids ) && ( ! empty( $all_lesson_ids ) || ! empty( $all_assignment_ids ) || ! empty( $all_quiz_ids ) ) ) {
			$reading_keys = array_map( fn( $cid ) => '_tutor_reading_info_' . $cid, $course_ids );
			$ri_ph        = implode( ',', array_fill( 0, count( $reading_keys ), '%s' ) );
            // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
			$reading_rows = $wpdb->get_results(
				$wpdb->prepare(
			...
			$attempt_rows = $wpdb->get_results(
				$wpdb->prepare(
                 ...
                 AND   attempt_status != %s",
					...array_merge( $user_ids, $all_quiz_ids, array( 'attempt_started' ) )
				)
			);
		...
		if ( ! empty( $all_assignment_ids ) ) {
            ...
            // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
			$submission_rows = $wpdb->get_results(
				$wpdb->prepare(
					"SELECT DISTINCT post_author, post_parent
                 FROM {$wpdb->posts}
                 WHERE post_parent IN ({$a_ph})
                 AND   post_author IN ({$u_ph})
			...
			$table_exists = (bool) $wpdb->get_var( $wpdb->prepare( 'SHOW TABLES LIKE %s', $cb_table ) );
		if ( $table_exists ) {
			// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
			$cb_rows = $wpdb->get_results(
				$wpdb->prepare(
			...
			foreach ( $cb_rows as $row ) {
				$uid = (int) $row->user_id;
				$lid = (int) $row->content_id;
				if ( isset( $all_lesson_set[ $lid ] ) ) {
					$lesson_done[ $uid ][ $lid ] = true;
				} elseif ( isset( $all_assign_set[ $lid ] ) ) {
					$assignment_done[ $uid ][ $lid ] = true;
				} elseif ( isset( $all_quiz_set[ $lid ] ) ) {
					$quiz_done[ $uid ][ $lid ] = true;
				}
			}
		}
		...
		foreach ( $user_ids as $uid ) {
			foreach ( $course_ids as $cid ) {
				...
				$lessons     = $course_lessons[ $cid ] ?? array();
				...
				if ( ! $diag_saved && $has_assignments ) {
					...
					$assign_ids_for_course = $course_assignments[ $cid ] ?? array();
					$cb_assign_rows        = array();
					if ( ! empty( $assign_ids_for_course ) ) {
						$ca_ph = implode( ',', array_fill( 0, count( $assign_ids_for_course ), '%d' ) );
                        // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
						$cb_assign_rows = $wpdb->get_results(
							$wpdb->prepare(
					...
					$assign_child_rows = array();
					$assign_def_ids    = $course_assignments[ $cid ] ?? array();
					if ( ! empty( $assign_def_ids ) ) {
						$ad_ph = implode( ',', array_fill( 0, count( $assign_def_ids ), '%d' ) );
                        // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
						$assign_child_rows = $wpdb->get_results(
							$wpdb->prepare(
					...
					set_transient(
						...
						array(
							'user_id'             => $uid,
							'course_id'           => $cid,
							'content_lessons'     => $course_lessons[ $cid ] ?? array(),
							'content_quizzes'     => $course_quizzes[ $cid ] ?? array(),
							'content_assigns'     => $course_assignments[ $cid ] ?? array(),
							'total'               => $total,
							'done'                => $done,
							'lesson_done_ids'     => array_keys( $lesson_done[ $uid ] ?? array() ),
							'assignment_done_ids' => array_keys( $assignment_done[ $uid ] ?? array() ),
							'cb_sample'           => $cb_sample,
